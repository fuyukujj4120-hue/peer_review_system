...
def student_review_panel():
    ...
    with st.form(f"review_{session['session_id']}"):
        score = st.slider("整體評分", 1, 5, 3)
        comment = st.text_area(
            "評語",
            placeholder="請填寫優點、改善建議或想提問的內容。",
            max_chars=1000,
        )
        send = st.form_submit_button("提交互評", type="primary")
    if send:
        try:
            submit_review(session["session_id"], score, comment)
            st.success("互評已提交。")
            st.rerun()
        except ValueError as error:
            st.error(str(error))
...
def student_results():
    ...
    show_export(grade_rows, "我的管理員評分.csv", "my_grades")
...
def admin_grading():
    ...
    st.subheader("全部管理員評分")
    show_export(grade_rows, "全學期管理員評分.csv", "all_grades")
...
def main():
    ...
    admin_route = st.query_params.get("page") == "admin"
